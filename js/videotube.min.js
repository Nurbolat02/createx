...
const init = () => {
	...
	const sizeVideo = () => {
		const sizeBlock = sizeBlockList.find(item => item[0] < window.visualViewport.width) || sizeBlockList[sizeBlockList.length - 1];
		const iframe = document.getElementById("videotube-modal");
		// Уменьшите ширину и высоту
		iframe.width = sizeBlock[0] / 2;
		iframe.height = sizeBlock[1] / 2;
		video.style.cssText = `
			width: ${sizeBlock[0] / 2}px;
			height: ${sizeBlock[1] / 2}px;
		`;
	};
