...
const passwordImg = document.querySelector('.password__img');
const passwordType = document.querySelector('.password');
const openModal = document.querySelector('.sign__in');
const modalWindow = document.querySelector('.modal');
const crossModal = document.querySelector('.close');
const mainNavLinks = document.querySelectorAll('.naviagtion__link');
const subject = document.querySelectorAll('.courses__subject');
const filter = document.querySelectorAll('.filter__info');
const tabBtns = document.querySelectorAll('.tabs__btn');
const tabContentItems = document.querySelectorAll('.tabs__content .tabs__pane');
const flexChangeBtn = document.querySelectorAll('.our-events__select svg');
const ourEvents = document.querySelector('.our-events')
const PagScroll = 180;
const goTopScroll = 0;
...
window.addEventListener('load', () => {
    ...
    mainNavLinks.forEach(function (link, index) {
        ...
        if (link.href.split('/').pop() == pageTitle) {
            link.classList.add('active')
        }
    })
});
goTop.addEventListener('click', () => {
    scrollToTop(goTopScroll)
});
function changeFlexToGrid(event) {
    let svgElement = event.target;
    console.log(svgElement)
    let clickPath = svgElement.querySelector('g')

    let x = clickPath.getAttribute('clip-path')
    if (x == 'url(#clip0_2313_4736)') {
        ourEvents.classList.remove('change-grid')
        flexChangeBtn[0].classList.add('svg-active')
        flexChangeBtn[1].classList.remove('svg-active')

    }
    else {
        ourEvents.classList.add('change-grid')
        flexChangeBtn[1].classList.add('svg-active')
        flexChangeBtn[0].classList.remove('svg-active')
    }
}
...
//     if (link.href.split('/').pop() === pageTitle) {
//         console.log(mainNavLinks[index]);
//     }
// });
// mainNavLinks.forEach(link => console.log(link.href.split('/').pop()))

passwordImg.addEventListener('click', changeSvg)
openModal.addEventListener('click', modelHandler)
crossModal.addEventListener('click', closeModal)
tabBtns.forEach(x => x.addEventListener('click', event => collorBtn(event)));
filter.forEach(btn => {
    btn.addEventListener('click', btnActive)
})
subject.forEach(function (sub) {
    let name = sub.innerHTML;
    arr.push(name)
})
...
filter.forEach(function (filterElement) {
    let filterInner = filterElement.innerHTML;
    if (result[filterInner] != undefined) {
        let span = document.createElement('span');
        span.innerHTML = result[filterInner];
        span.className = "filter__number";
        filterElement.append(span)
    }
    else {
...
function btnActive(event) {
    let x = event.target.dataset.subject;

    subject.forEach(sub => {
        if (x == 'all') {
            sub.closest('.courses__element').classList.remove('none')
        }
        else if ((sub.classList.contains(x))) {
            sub.closest('.courses__element').classList.remove('none')

        }
        else {
            sub.closest('.courses__element').classList.add('none')
        }
    })
}
window.onscroll = function show_header() {
    const header = document.querySelector('.header__nav nav')
...
let futureDate = currentDate.setMonth(currentDate.getMonth() + 1);
// как запускать это код только на определенной странице?
const timerElement = document.querySelector(".discount__time");

if (timerElement !== null) {
    // Если элемент с таймером существует, то выполняем код для обновления счетчика времени
    ...
    // Устанавливаем интервал для обновления счетчика времени
    ...
    function closeAccTabs() {
        for (let i = 0; i < acc.length; i++) {
            if (acc[i].classList.contains("active")) {
                ...
                //  мы убираем этот класс у кнопки
                acc[i].nextElementSibling.removeAttribute("style");
                // и у следуйщего элемента, нашей панели
                // которая идет после кнопки, мы убираем стили
            }
        }
    }
}
...
const listItems = document.querySelectorAll(".our-events .events__list .event__item");
...
// разбери как он работает ебаный ты в рот
